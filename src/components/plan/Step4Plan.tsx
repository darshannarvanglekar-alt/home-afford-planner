...
type CorpusRoute = "monthlyInvestment" | "fixedSaving" | "lumpSum" | "pooledSaving" | "gold" | "recurringDeposit";
...
const routeCards: Array<{ id: CorpusRoute; title: string; description: string; icon: React.ElementType; defaultRate: number; monthly: boolean; stepEnabled: boolean }> = [
  { id: "monthlyInvestment", title: "Monthly Investment", description: "A monthly market-linked route using your assumed return.", icon: TrendingUp, defaultRate: 10, monthly: true, stepEnabled: true },
  { id: "fixedSaving", title: "Monthly Fixed Return Saving", description: "A monthly fixed-return saving route using your assumed return.", icon: Landmark, defaultRate: 7, monthly: true, stepEnabled: true },
  { id: "lumpSum", title: "Lump Sum Investment", description: "A one-time amount that grows until possession or purchase.", icon: Banknote, defaultRate: 8, monthly: false, stepEnabled: false },
  { id: "pooledSaving", title: "Chit-Style Pooled Saving", description: "A monthly pooled saving route using your assumed return.", icon: WalletCards, defaultRate: 9, monthly: true, stepEnabled: false },
  { id: "gold", title: "Gold Accumulation", description: "A monthly gold accumulation route using your assumed return.", icon: Coins, defaultRate: 8, monthly: true, stepEnabled: false },
  { id: "recurringDeposit", title: "Recurring Deposit", description: "A monthly recurring deposit route using your assumed return.", icon: Layers3, defaultRate: 7, monthly: true, stepEnabled: false },
];

type RouteAllocation = { id: CorpusRoute; amount: number; rate: number; stepUp: boolean };

function CorpusBuilder({ finances, investments, home, planSurplus, onSafetyAllocation }: { finances: Finances; investments: CurrentInvestment[]; home: Home; planSurplus: number; onSafetyAllocation: (amount: number) => void }) {
  const targetDefault = Math.max(100000, home.downPayment + home.registrationStampDuty + home.interiorBudget);
  const existing = React.useMemo(() => summarizeInvestments(investments, home.possessionMonth), [investments, home.possessionMonth]);
  ...
  const [allocations, setAllocations] = React.useState<RouteAllocation[]>([
    { id: "monthlyInvestment", amount: Math.max(0, Math.round(Math.min(planSurplus, 25000))), rate: 10, stepUp: false },
  ]);
  ...
  const updateMarketRates = (rate: number) => {
    setAllocations(allocations.map((item) => (item.id === "monthlyInvestment" ? { ...item, rate } : item)));
  };
    ...
    <>
      <section className="rounded-2xl border border-border bg-card p-5 shadow-soft sm:p-6">
        ...
        <div className="mt-5 rounded-2xl border border-primary/20 bg-primary-soft p-4 text-sm font-semibold text-primary-soft-foreground">
          Your existing investments are already building {formatINR(existing.projectedCorpus)} toward your target. You need {formatINR(Math.max(0, target - existing.projectedCorpus))} more.
        </div>

        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          <MetricCard label="Current Savings" value={formatINR(existing.currentCorpus)} />
          <MetricCard label="Existing Corpus by Target Date" value={formatINR(existing.projectedCorpus)} valueClassName="text-success" />
          <MetricCard label="Additional Corpus Needed" value={formatINR(additionalNeeded)} />
        </div>

        <div className="mt-6">
          <h3 className="text-base font-extrabold text-foreground">How do you want to build the additional corpus needed?</h3>
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            {routeCards.map((card) => {
              const Icon = card.icon;
              const isSelected = selected.has(card.id);
              return (
                <button key={card.id} type="button" className={cn("relative rounded-2xl border bg-background p-4 text-left transition-all", isSelected ? "border-primary ring-2 ring-primary/15" : "border-border hover:border-primary/50")} onClick={() => toggleRoute(card.id)}>
                  {isSelected ? <span className="absolute right-3 top-3 flex h-6 w-6 items-center justify-center rounded-full bg-primary text-primary-foreground"><Check className="h-4 w-4" /></span> : null}
        ...
        <div className="sticky top-24 z-10 mt-6 rounded-2xl border border-border bg-background/95 p-4 shadow-soft backdrop-blur">
          ...
          <p className={cn("mt-2 text-sm font-medium", totalMonthly > planSurplus ? "text-warning" : "text-success")}>
            {totalMonthly > planSurplus ? `Your total allocation exceeds your available surplus by ${formatINR(surplusDelta)}. Consider adjusting the amounts.` : `You have ${formatINR(surplusDelta)} unallocated surplus remaining.`}
          </p>
        </div>
        ...
        <div className="mt-6 space-y-5 rounded-2xl border border-border bg-muted/25 p-4">
          ...
          <ScenarioChips rates={[8, 10, 12]} selected={allocations.find((item) => item.id === "monthlyInvestment")?.rate ?? 10} onSelect={updateMarketRates} />
          <div className="space-y-3">
            {allocations.map((allocation) => <AllocationRow key={allocation.id} allocation={allocation} onChange={(patch) => updateAllocation(allocation.id, patch)} />)}
          </div>
        </div>

        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          <MetricCard label="Combined Projected Corpus" value={formatINR(result.final)} valueClassName="text-primary" />
        ...
        <div className="mt-5 space-y-2 rounded-2xl border border-border bg-background p-4">
          {result.breakdown.map((item) => <div key={item.id} className="flex items-center justify-between gap-4 text-sm"><span className="font-medium text-muted-foreground">{routeTitle(item.id)}</span><span className="font-extrabold text-foreground">{formatINR(item.value)}</span></div>)}
        </div>
        ...
        <div className="mt-5 h-64 rounded-2xl border border-border bg-background p-3">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={result.points} margin={{ top: 10, right: 8, left: 0, bottom: 0 }}>
              ...
              <Line type="monotone" dataKey="total" name="Combined corpus" stroke="var(--color-chart-1)" strokeWidth={3} dot={false} />
              <Line type="monotone" dataKey="target" name="Target corpus" stroke="var(--color-chart-3)" strokeWidth={2} dot={false} strokeDasharray="5 5" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      ...
      <SafetyBufferPlanner finances={finances} defaultMonthly={Math.max(0, Math.round(totalMonthly * 0.25))} onInclude={onSafetyAllocation} />
    </>
  );
}

function AllocationRow({ allocation, onChange }: { allocation: RouteAllocation; onChange: (patch: Partial<RouteAllocation>) => void }) {
  const card = routeCards.find((item) => item.id === allocation.id)!;
  const Icon = card.icon;
  return (
    <div className="rounded-2xl border border-border bg-background p-4">
      <div className="flex items-center gap-3"><Icon className="h-5 w-5 text-primary" /><p className="font-extrabold text-foreground">{card.title}</p></div>
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        <CurrencyField label={card.monthly ? "Monthly contribution amount" : "Lump sum amount"} value={allocation.amount} onChange={(amount) => onChange({ amount })} />
        <NumberField label="Assumed return rate (%)" value={allocation.rate} onChange={(rate) => onChange({ rate })} />
      </div>
      {card.stepEnabled ? <div className="mt-4"><StepUpControl enabled={allocation.stepUp} setEnabled={(stepUp) => onChange({ stepUp })} value={10} onChange={() => undefined} /></div> : null}
    </div>
  );
}
...
function isMonthlyRoute(route: CorpusRoute) { return route !== "lumpSum"; }
function routeTitle(route: CorpusRoute) { return routeCards.find((item) => item.id === route)?.title ?? route; }
