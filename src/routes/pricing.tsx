...
export const Route = createFileRoute("/pricing")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Simple HomeAfford pricing with one free plan and Pro AI planning.",
      },
    ],
  }),
      ...
      <main className="flex-1">
        <section className="mx-auto w-full max-w-6xl px-4 py-16 text-center sm:px-6 sm:py-20">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-muted-foreground">
            Start free, then unlock unlimited AI-assisted planning when you're ready.
          </p>
        </section>
        ...
        <section className="mx-auto grid w-full max-w-5xl gap-5 px-4 sm:px-6 md:grid-cols-2">
          <PlanCard
            title="Free"
            price="₹0"
            subtitle="1 saved plan included"
            features={["1 saved plan", "Basic verdict", "Manual entry"]}
          />
          <PlanCard
            title="Pro"
            price="₹499/mo"
            subtitle="or ₹3,999/year"
            features={[
        ...
        <section className="bg-muted/40">
          <div className="mx-auto grid w-full max-w-5xl gap-4 px-4 py-16 sm:px-6 md:grid-cols-3">
            <Faq
              ...
              a="Yes. Your statements are analysed instantly and permanently deleted. We never store your financial files."
            />
            <Faq
            ...
            <Faq
              ...
              a="Yes — 1 complete plan is always free. No credit card required."
            />
          </div>
        </section>
      </main>
...
function PlanCard({
  ...
  return (
    <article
      className={`rounded-2xl border bg-card p-6 shadow-soft ${featured ? "border-primary ring-2 ring-primary/15" : "border-border"}`}
    >
      {featured && <Badge>Popular</Badge>}
      <h2 className="mt-2 text-2xl font-extrabold text-foreground">{title}</h2>
      <p className="mt-3 text-3xl font-extrabold text-primary">{price}</p>
